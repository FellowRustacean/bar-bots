...
async function respondAsSinglePersona({
  ...
  // format: 'json' jetzt IMMER (Getraenke-Bestellung ist fest im System-Prompt verankert, siehe
  // buildOrderInstructions) - jeder Charakter antwortet im selben JSON-Format, bestellung/getraenk
  // werden aber nur bei Quinn im Tresen-Kanal ausgewertet (siehe unten).
  ...
  // eigenen Tokenizer - siehe measureAndCacheSystemBaseline in ollamaClient.js. Da SYSTEM_PROMPT
  // jetzt komplett statisch ist, gibt es effektiv nur EINEN Cache-Eintrag, der einmal pro Neustart
  // gefuellt wird. NICHT fire-and-forget: der Messungs-Call teilt sich die "busy"-Sperre in
  // ollamaClient.js mit echten Anfragen - liefe er unbeobachtet im Hintergrund weiter, waehrend
  // runJob() schon den naechsten Charakter derselben Runde aufruft, wuerde dessen echter Call auf
  // die Sperre treffen und fehlschlagen. Kostet dadurch nur beim ALLERERSTEN Call nach einem
  // Neustart einmalig etwas mehr Zeit - danach liefert der Cache die echte Zahl sofort.
  let systemTokenEstimate = getCachedSystemBaseline(SYSTEM_PROMPT);
  if (systemTokenEstimate == null) {
    systemTokenEstimate = (await measureAndCacheSystemBaseline(SYSTEM_PROMPT, target).catch(() => null)) ?? estimateTokens(SYSTEM_PROMPT);
  }
  ...
  const answerText = typeof parsed?.antwort === 'string' ? parsed.antwort : raw;
  const cleaned = cleanPersonaReply(answerText, persona.displayName);
  ...
  if (persona.botName === 'barkeeper' && channelId === getBartresenChannelId(guildId)) {
    const rawDrinkName = typeof parsed?.getraenk === 'string' ? parsed.getraenk.trim() : null;
    const validDrink = matchDrink(rawDrinkName, getDrinkNames());
