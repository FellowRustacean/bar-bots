...
function leaveChannel(guildId) {
  ...
  if (connection) {
    // destroy() wirft, wenn die Verbindung bereits zerstoert ist (z.B. weil Discord sie von
    // sich aus schon getrennt hat, oder ein ueberlappender Poll-Tick sie schon abgeraeumt hat -
    // siehe Bugfix-Notiz bei processDueLeaves) - idempotent machen, das ist hier kein Fehler.
    try {
      connection.destroy();
    } catch {
      // schon weg - genau das wollten wir erreichen.
    }
...
async function processDueLeaves(botName) {
  ...
  // markLeft() MUSS in jedem Fall laufen, auch wenn leaveChannel() wirft - sonst bleibt der
  // Eintrag "due" (left=0) und wird beim naechsten Poll-Tick (alle 5s) erneut versucht: genau
  // das fuehrte am 01.09. zu einer 8-minuetigen Fehlerspam-Schleife im Error-Log (immer
  // derselbe Teilnehmer, "Cannot destroy VoiceConnection - it has already been destroyed").
  // Gleiches Muster wie in processDueJoins oben - dort ist markJoined() bereits so abgesichert.
  for (const job of dueLeaves) {
    try {
      leaveChannel(job.guildId);
    } catch (err) {
      await logError(err, { context: 'Team-Päuschen: Verlassen', guildId: job.guildId });
    } finally {
...
// runAsLeader kann bis zu TABLE_WAIT_MAX_TRIES * TABLE_WAIT_RETRY_MS = 15s auf einen Tisch
// warten - laenger als POLL_INTERVAL_MS (5s). Ohne diese Sperre wuerde setInterval trotzdem
// weiter alle 5s neue, ueberlappende Durchlaeufe anstossen, waehrend ein Leader noch wartet -
// eine plausible Mitursache fuer die Doppel-destroy()-Situation, die zur Fehlerspam-Schleife
// vom 01.09. fuehrte (siehe processDueLeaves). Einfache Reentrancy-Sperre statt echter Queue,
// weil ein uebersprungener Tick beim naechsten Mal ohnehin nachgeholt wird (getDueJoins/
// getDueLeaves lesen den aktuellen DB-Stand, es geht nichts verloren).
...
function startTeamMeetingPoller(client, botName) {
  setInterval(async () => {
    ...
    pollRunning = true;
    try {
      await processDueJoins(client, botName).catch((err) =>
        logError(err, { context: 'Team-Päuschen-Poller (Beitritte)' })
      );
      await processDueLeaves(botName).catch((err) =>
