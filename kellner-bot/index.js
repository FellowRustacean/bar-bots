...
const { handleVoiceStateUpdate } = require('./utils/customVoice/customVoice');
const { deleteVoiceChannel } = require('./storage/voiceChannels');
...
const { startSnackDeliveryScheduler } = require('./utils/snackDelivery/scheduler');
...
const { handleVoiceLog } = require('./utils/voiceLog/voiceLog');
...
const { handleHelpButtonClick } = require('../shared/lib/helpButton');
// Cross-Prozess-Import - manager-bot besitzt die XP-Logik (adjustTotalXp), siehe Session-
// Historie 02.09. (aehnlicher Trick schon fuer den preloadModel-Systemprompt-Fix genutzt).
// adjustTotalXp umgeht bewusst das Tages-/Monatslimit (DAILY_CAP) - Nutzerwunsch.
const { adjustTotalXp } = require('/home/pi/bots/manager-bot/storage/xp.js');

const dmThreadFlow = createDmThreadFlow({
  botName: 'kellner',
  label: 'Benedict (Kellner)',
  getDmThread,
  getDmThreadByTarget,
  createDmThread,
  getLogChannel,
...
client.once('clientReady', () => {
  ...
  startOutboxPoller(client, 'kellner');
  startInteractionScheduler(client, 'kellner');
  startSnackDeliveryScheduler(client);
  startStatusRotator(client, statusMessages);
  startTeamMeetingPoller(client, 'kellner');
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    if (interaction.customId.startsWith('dm_confirm:') || interaction.customId.startsWith('dm_cancel:')) {
      try {
      ...
      try {
        await handleHelpButtonClick(interaction, adjustTotalXp);
      } catch (err) {
        await replyWithError(interaction, err);
      }
    }
...
// Faengt auch Loeschungen ab, die NICHT ueber die eigene "Channel ist leer"-Logik in
// customVoice.js liefen (z. B. ein Tisch wurde manuell ueber Discord geloescht) - sonst bleibt
// die zugehoerige voice_channels-Zeile fuer immer stehen und "belegt" die Tischnummer weiterhin,
// obwohl der Channel gar nicht mehr existiert (fuehrt zu immer weiter hochzaehlenden Tisch-
// Nummern trotz weniger tatsaechlich existierender Tische). Reines DELETE ohne Treffer ist ein
// No-Op, daher hier bedenkenlos fuer JEDEN geloeschten Channel aufgerufen, nicht nur Tische.
client.on('channelDelete', (channel) => {
  try {
    deleteVoiceChannel(channel.id);
  } catch (err) {
    logError(err, { context: 'Verwaiste Tisch-Zeile aufraeumen', guildId: channel.guild?.id }).catch(() => {});
  }
});
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  try {
    ...
    await logError(err, { context: 'Custom-Voice-System', guildId });
  }

  try {
    await handleVoiceLog(oldState, newState);
  } catch (err) {
    await logError(err, { context: 'Voice-Join/Leave-Log', guildId });
  }
});
