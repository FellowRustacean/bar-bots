...
const { getGameByVoiceChannel } = require('./utils/werwolf/gameManager');
const { handlePlayerLeftVoice } = require('./utils/werwolf/gameRunner');
...
client.once('clientReady', () => {
  ...
  startOutboxPoller(client, 'werwolf');
  startInteractionScheduler(client, 'werwolf');
  startStatusRotator(client, statusMessages);
  startTeamMeetingPoller(client, 'werwolf');
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    try {
      if (interaction.customId === 'werwolf_hexe_protect') {
        await client.commands.get('werwolf').handleHexeProtectButton(interaction);
      } else if (interaction.customId === 'werwolf_skip_turn') {
        await client.commands.get('werwolf').handleSkipTurnButton(interaction);
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId.startsWith('werwolf_cancel_modal:')) {
      try {
        await client.commands
          .get('werwolf')
          .handleCancelModalSubmit(interaction);
      } catch (err) {
        await replyWithError(interaction, err);
      }
    }
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  try {
    if (oldState.channelId && oldState.channelId !== newState.channelId) {
      const game = getGameByVoiceChannel(oldState.channelId);
      if (game && (game.phase === 'night' || game.phase === 'day')) {
        await handlePlayerLeftVoice(client, game, oldState.member.id);
      }
    }
  } catch (err) {
    await logError(err, { context: 'Werwolf-Voice-Tracking', guildId });
  }
});
